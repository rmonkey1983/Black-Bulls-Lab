                ...
                <div ref={stepContainerRef} className="relative">
                    ...
                    {step === 2 && (
                        <div key="step2" className="max-w-md mx-auto space-y-8">
                            <div>
                                <p className="font-mohave uppercase tracking-widest text-white/60 text-sm text-center mb-6">Passo 2 — Quante persone partecipano e quando?</p>
                                
                                <div className="space-y-4">
                                    <div className="flex justify-between items-end">
                                        <label className="font-mohave uppercase text-white text-sm tracking-widest">Numero Persone</label>
                                        <span className="text-rama-accent font-mohave font-bold text-3xl">{numPeople}</span>
                                    </div>
                                    <input 
                                        type="range" 
                                        min="10" 
                                    ...
                                    />
                                    <div className="flex justify-between text-[10px] font-outfit text-white/30 uppercase tracking-widest">
                                        <span>Min 10</span>
                                        <span>Max 100</span>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-4">
                                <label className="font-mohave uppercase text-white text-sm tracking-widest block">Data Preferita (Opzionale)</label>
                                <input 
                                    type="date" 
                                    value={preferredDate}
                                    onChange={(e) => setPreferredDate(e.target.value)}
                                    className="w-full bg-white/[0.03] border border-white/10 rounded-lg p-4 font-outfit text-white focus:border-rama-accent/50 outline-none transition-colors [color-scheme:dark]"
                                />
                            </div>
                            ...
                            <div className="flex flex-col gap-4 mt-10">
                                <button 
                                    onClick={() => setStep(3)}
                                    className="w-full bg-rama-accent text-black font-mohave font-bold uppercase tracking-widest text-sm px-8 py-4 rounded-xl hover:bg-white transition-all duration-300 shadow-[0_4px_20px_rgba(200,164,78,0.2)]"
                                >
                                    Scopri i Format →
                                </button>
                    ...
                    {step === 3 && (
                        <div key="step3">
                            ...
                            <button onClick={() => { setStep(1); setSelectedOccasion(null); setNumPeople(25); setPreferredDate(""); }} className="text-white/40 hover:text-white font-outfit text-sm underline underline-offset-4 transition-colors mx-auto block">
