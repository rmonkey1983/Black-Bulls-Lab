...
export function RamaServices() {
    ...
    return (
        <section className="relative w-full min-h-screen bg-rama-bg flex items-center justify-center py-32 overflow-hidden px-6 md:px-12">
            ...
            <div className="relative z-10 w-full flex flex-col items-center">
                <div className="font-rock-salt text-rama-accent transform -rotate-2 text-xl mb-12">I Nostri Servizi</div>

                <div className="flex flex-col w-full max-w-5xl">
                    {services.map((service) => (
                        <div
                            key={service.id}
                            className={`group border-b border-white/10 py-6 md:py-10 flex flex-col md:flex-row items-start md:items-center cursor-pointer transition-colors duration-500`}
                            onMouseEnter={() => setHoveredService(service.id)}
                        >
                            <span className="text-white/30 text-lg md:text-2xl font-mohave group-hover:text-rama-accent transition-colors w-16 md:w-32">
                                ({service.id})
                            </span>
                            <h2
                                className={`font-mohave text-[10vw] md:text-[8vw] uppercase font-bold tracking-tighter leading-[0.85] transition-all duration-500 ${hoveredService === service.id ? 'text-white' : 'text-transparent'}`}
                                style={{
