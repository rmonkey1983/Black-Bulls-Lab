...
export function ProssimeDate({ events }: ProssimeDateProps) {
  ...
  return (
    <section id="prossime-date" className="reveal-section section-padding bg-black-pure border-t border-white/5">
      <div className="container-max">
        <SectionHeading
          title="PROSSIME"
          highlight="DATE"
          subtitle="Upcoming Experience Sessions"
          align="center"
        />
          ...
          {events.map((event) => {
            ...
            return (
              <Link
                ...
                className="group relative flex flex-col justify-between p-10 lg:p-12 border border-white/5 bg-black-elevated/10 rounded-none backdrop-blur-3xl transition-all duration-1000 ease-[cubic-bezier(0.19,1,0.22,1)] hover:border-accent-gold/20 hover:shadow-[0_40px_100px_rgba(0,0,0,0.6)] stagger-item"
              >
                {/* Spots Badge */}
                <div className="absolute top-8 right-8">
                  <span className={`inline-flex items-center gap-2 px-4 py-1.5 border text-[9px] font-syne font-bold uppercase tracking-[0.4em] ${
                    ...
                    {isSoldOut ? "Sold Out" : `${event.available_slots} Slots Left`}
                  </span>
                </div>
                ...
                <div className="space-y-12">
                  ...
                  <p className="font-inter text-xs uppercase tracking-[0.3em] text-text-secondary opacity-40">
                    Admission: <span className="text-text-primary font-bold">€{event.price || 50}</span>
                  </p>
                </div>

                {/* CTA */}
                <div className="pt-12 mt-12 flex items-center justify-between border-t border-white/5">
                  <span className="font-syne text-[9px] font-bold uppercase tracking-[0.4em] text-text-secondary group-hover:text-accent-gold transition-colors duration-700">
                    {isSoldOut ? "View Details" : "Secure Entry"}
                  </span>
                  <div className={`w-10 h-10 flex items-center justify-center border border-accent-gold/20 transition-all duration-700 ${
        ...
        <div className="mt-24 lg:mt-40 text-center">
          ...
          >
            ...
            <ArrowRight size={16} strokeWidth={1.5} className="text-text-secondary group-hover:text-accent-gold group-hover:translate-x-2 transition-all duration-700" />
          </a>
        </div>
      </div>
    </section>
