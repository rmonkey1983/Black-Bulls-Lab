...
export function useCinematic() {
  ...
  const staggerReveal = (parentSelector: string, childSelector: string) => {
    if (
      !registrationsRef.current.some(
        (r) => r.type === "stagger" && r.args[0] === parentSelector && r.args[1] === childSelector
      )
    ) {
    ...
    const ctx = gsap.context(() => {
      registrationsRef.current.forEach((reg) => {
        if (reg.type === "reveal") {
          ...
          const parent = typeof parentSelector === "string" ? document.querySelector(parentSelector) : null;
          if (!parent) return;
