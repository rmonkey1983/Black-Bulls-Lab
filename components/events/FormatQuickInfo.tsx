...
export function FormatQuickInfo({ duration, capacity, price, highlight, highlightLabel }: FormatQuickInfoProps) {
    ...
    return (
        <section className="w-full py-12 px-4 md:px-6 bg-white/2 border-y border-white/5">
            <div className="max-w-7xl mx-auto">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
                    {infos.map((info, i) => (
                        <div key={i} className="flex flex-col gap-2 p-4 rounded-xl bg-white/3 border border-white/5 hover:border-rama-accent/30 transition-colors">
